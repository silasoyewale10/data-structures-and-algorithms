...
const filterStringsWithVowels = (arr) => {
    // console.log('gregor'.includes('e'))
    return arr.filter(item => item.includes('a') || item.includes('e') || item.includes('i') ||item.includes('o') || item.includes('u'))
    // console.log(item)
  // Solution code here...
};
...
const getStatName = (arr, minBaseStat) => {
    // console.log(arr[0].stat.name)
   var newArray = arr.filter(item => {
       return item.baseStat > minBaseStat;
   })
   return newArray.map(item => item.stat.name)
};
